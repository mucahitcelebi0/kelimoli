...
public class AdMobPlugin: CAPPlugin, CAPBridgedPlugin {
    ...
    /**
     * PATCHED for Apple App Review 2026-06-10 (Build 1.0.5):
     * - Wrapped in DispatchQueue.main.async (Apple iOS 18+ / iPadOS 26.5
     *   enforces main thread for ATTrackingManager.requestTrackingAuthorization;
     *   otherwise prompt is silently suppressed).
     * - Returns the resolved status to JS instead of empty dict.
     */
        ...
        if #available(iOS 14, *) {
            ...
            DispatchQueue.main.async {
                ATTrackingManager.requestTrackingAuthorization { status in
                    ...
                    switch status {
                    case .authorized:
                        statusString = AuthorizationStatusEnum.Authorized.rawValue
                    case .denied:
                        statusString = AuthorizationStatusEnum.Denied.rawValue
                    case .restricted:
                        statusString = AuthorizationStatusEnum.Restricted.rawValue
                    case .notDetermined:
                        statusString = AuthorizationStatusEnum.NotDetermined.rawValue
                    @unknown default:
                        statusString = AuthorizationStatusEnum.NotDetermined.rawValue
                    }
